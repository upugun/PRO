var meetingApp = angular.module("bookingModule", [ 'ui.bootstrap' ]);
...
var bookingController = [
		...
		function($scope, $cookieStore, $rootScope, $http, constantService,
			...
			$scope.getBookingByStatus = function() {
				fetch();
			}

			function fetch() {
				var q = {
					"searchCode" : "searchByStatus",
			...
			function insertObj(obj) {
				...
				$http.post(constants.apiUrl + 'booking/insert', {
				...
				for (var i = 0; i < $scope.facilities.length; i++) {
					if ($scope.facilities[i].selectedStatus == true) {
						var obj = {
							...
							"count":$scope.facilities[i].count,
							"status" : "ACTIVE",
							"facilities":$scope.facilities[i],
						};
						list.push(obj);
...
meetingApp.controller("bookingController", bookingController);